#!/usr/bin/env python
# -*- coding:utf-8 -*-
"""
@author: GW
@time: 2025-06-03 23:04 
@file: get_smf_select_data.py
@project: 5gAPItest
@describe: Powered By GW
"""
# 文件: sbi_client/udm/get_smf_select_data.py

from core_5g_sbi_test.core_sbi_messages.sbi_base_processor import SBIBaseProcessor
import json
import time
from email.utils import formatdate


class GetSmfSelectDataProcessor(SBIBaseProcessor):
    def __init__(self, connection, ):
        super().__init__(connection)

    def sender(self):
        return "AMF"

    def receiver(self):
        return "UDM"

    def build_headers(self, body=None):
        return [
            (':method', 'GET'),
            (':path', '/nudm-sdm/v2/imsi-001010000000000/smf-select-data'
                      '?plmn-id=%7B%22mcc%22%3A%22001%22%2C%22mnc%22%3A%2201%22%7D'),
            (':scheme', 'http'),
            (':authority', f'{self.connection.host}:{self.connection.port}'),
            ('accept', 'application/json,application/problem+json'),
            ('3gpp-sbi-sender-timestamp', 'Tue, 03 Jun 2025 02:41:42.771 GMT'),
            ('3gpp-sbi-max-rsp-time', '10000'),
            ('user-agent', 'AMF')
        ]

    def build_body(self):
        return b''  # GET 请求没有 body

    def match(self, path: str) -> bool:
        return (
                path.startswith("/nudm-sdm/v2/imsi-")
                and "/smf-select-data" in path
        )

    def handle(self, stream_id, conn, headers: list):
        response_body_dict = {
            "subscribedSnssaiInfos": {
                "1": {
                    "dnnInfos": [
                        {
                            "dnn": "internet",
                            "defaultDnnIndicator": True
                        }
                    ]
                }
            }
        }

        response_body = json.dumps(response_body_dict).encode("utf-8")

        response_headers = [
            (':status', '200'),
            ('server', 'Open5GS v2.7.5-5-g1182a99'),
            ('date', formatdate(timeval=None, localtime=False, usegmt=True)),
            ('content-type', 'application/json'),
            ('content-length', str(len(response_body))),
        ]

        return response_headers, response_body

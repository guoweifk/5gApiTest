...
free5gc_ngsetup_data = bytes.fromhex(
    "00150041000004001b00090000f1105000000001005240140880554552414e53494d2d676e622d312d312d310066001000000000010000f110000010080102030015400140")
...
free5gc_ini_data = bytes.fromhex(
    "000f40480000050055000200010026001a197e004179000d0100f1100000000000000000102e04f0f0f0f0007900135000f110000000010000f110000001eb617019005a4001180070400100")
ini_data = open5gs_ini_data

# 目标服务器
TARGET_IP = "192.168.55.28"
TARGET_PORT = 38412
wait_time_start = 0.1
wait_time_end = 10

# 全局同步信号
gnb_ready = threading.Event()


# # gNB 启动线程
def sctp_gnb_thread(thread_id):
    sk = sctp.sctpsocket_tcp(socket.AF_INET)
    sk.connect((TARGET_IP, TARGET_PORT))  # 作为 client 连接 SCTP server
    print(f"[Thread {thread_id}] Connected and sending data...")

    if thread_id == 0:
        sk.sendall(ngsetup_data)
        response_1 = sk.recv(1024)
        print(f"[Thread {thread_id}] Received: {response_1.hex()}")
...
def sctp_client_thread(thread_id):
    ...
    while True:
        try:
            ...
            KSEAF, auth_response = authReceiveAndResult(auth_request.hex())
            print(f" Received security_request: {auth_response}")
            # time.sleep(wait_time)
            ...
            # print(f" Received security_request: {security_request.hex()}")
            security_response = securityReceiveAndResult(KSEAF, security_request.hex())
            # time.sleep(wait_time)
            sk.sendall(bytes.fromhex(security_response))
            # print(f"[Thread {thread_id}] Received security_resp")
            #
            # onse: {security_response}")
            # time.sleep(wait_time)
...
def sctp_client_thread1(thread_id):
...
def test_mulit():
    ...
    for i in range(num_threads):
        t = Thread(target=sctp_client_thread1, args=(i,))
        t.start()
        threads.append(t)
    k += 1
    print(f"batch = {k}")
